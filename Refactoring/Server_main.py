...
import config
from utils import send_json_line

# Ports
AGENT_CTRL_PORT = config.AGENT_CTRL_PORT
AGENT_IMG_PORT = config.AGENT_IMG_PORT
GUI_CTRL_PORT = config.GUI_CTRL_PORT
GUI_IMG_PORT = config.GUI_IMG_PORT

# Optional server-side saving (GUI도 저장하므로 기본 False 권장)
SAVE_ON_SERVER = False
OUT_DIR = config.get_server_output_dir()
...
class GuiControlHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        try:
            send_json_line(sock, {"event":"hello","agent_state":state})
        except: pass
            ...
            while True:
                ...
                while True:
                    ...
                    try:
                        ...
                        send_json_line(sock, {"event":"error","message":"bad json"}); continue

                    with agent_ctrl_lock:
                        agent = agent_ctrl_sock
                    if agent is None:
                        send_json_line(sock, {"event":"error","message":"no agent connected"})
                        continue

                    try:
                        send_json_line(agent, cmd)  # forward as-is
                        # Optionally ack GUI
                        send_json_line(sock, {"event":"ack","cmd":cmd.get("cmd","")})
                    except Exception as e:
                        send_json_line(sock, {"event":"error","message":f"forward failed: {e}"})
        finally:
            with gui_ctrl_lock:
