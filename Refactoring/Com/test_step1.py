...
class App:
    ...
    def _get_yolo_model(self):
        ...
        # 캐싱: 경로가 같으면 재사용
        if hasattr(self, '_cached_yolo_model') and self._cached_yolo_model is not None and self._cached_yolo_wpath == wpath:
            return self._cached_yolo_model
        
        # 새로 로드
        self._cached_yolo_model = YOLO(wpath)
        self._cached_yolo_wpath = wpath
        print(f"[YOLO] 모델 로드됨: {wpath}")
        return self._cached_yolo_model
    ...
    def _load_image_pair(self, path_on, path_off):
        """ON/OFF 이미지 쌍 로드"""
        img_on = self._load_image_from_file(path_on)
        img_off = self._load_image_from_file(path_off)
        return img_on, img_off
    ...
    def _handle_scan_start(self, evt):
        """Handle scan start event - setup progress bar and CSV"""
    ...
    def _handle_scan_done(self, evt):
        """Handle scan completion - start YOLO processing"""
        ...
        def process_diff_and_yolo():
            try:
                ...
                for (pan, tilt), files in sorted(pairs.items()):
                    ...
                    if processed % 10 == 0:
                        ui_q.put(("toast", f"[DIFF] {processed}/{total_pairs}"))
    ...
    def _handle_generic_saved_image(self, name, data):
        """Handle generic saved image (scan results)"""
