...
# 파일명 파싱용 정규표현식
# 예: img_t+00_p+000_..._led_on_ud.jpg
# t값, p값, on/off 상태를 추출합니다.
PATTERN = re.compile(r"img_t([+\-]\d+)_p([+\-]\d+)_.*_led_(on|off)(_ud)?\.(jpg|png|jpeg)", re.IGNORECASE)
...
def main():
    ...
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("1. '_ud' (보정된) 이미지가 들어있는 폴더를 선택하세요.")
    ...
    # _ud가 붙은 이미지 파일 검색
    files = list(in_path.glob("*_ud.*"))
    print(f"🔍 발견된 보정 이미지(_ud) 수: {len(files)}")

    if len(files) == 0:
        print("⚠️ '_ud'로 끝나는 이미지 파일이 없습니다. 보정 옵션을 켜고 스캔했는지 확인하세요.")
        return
