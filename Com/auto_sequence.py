...
class App:
    def __init__(self, root: Tk):
        ...
        # [NEW] Auto Sequence State
        # 0:IDLE, 1:SCAN, 2:COMPUTE, 3:MOVE, 4:CENTER, 5:POINT
        self.auto_seq_state = 0
        ...
        # [NEW] Auto Sequence Button
        Button(ops, text="▶ START AUTO SEQUENCE", command=self.start_auto_sequence, bg="lightblue", font=("Arial", 10, "bold"))\
            .pack(side="left", padx=4)
        ttk.Separator(ops, orient="vertical").pack(side="left", fill="y", padx=4)
        ...
        try:
            ...
            if abs(err_x) <= tol and abs(err_y) <= tol:
                ...
                if self._centering_stable_cnt >= self.centering_min_frames.get():
                    ...
                    # [AUTO] Step 4 Done -> Step 5
                    if self.auto_seq_state == 4:
                        self.root.after(1000, self._auto_seq_step_5_pointing)
                    return
        ...
        try:
            ...
            if abs(err_x) <= tol and abs(err_y) <= tol:
                ...
                if self._pointing_stable_cnt >= self.centering_min_frames.get():
                    ...
                    # [AUTO] Step 5 Done -> Finish
                    if self.auto_seq_state == 5:
                        self.auto_seq_state = 0
                        ui_q.put(("toast", "✨ Auto Sequence All Done! ✨"))
                    return
            else:
                self._pointing_stable_cnt = 0
                ...
                if tag == "evt":
                    ...
                    if et == "hello":
                        ...
                        def process_diff_and_yolo():
                            try:
                                ...
                                # [AUTO] Step 1 Done -> Step 2
                                if self.auto_seq_state == 1:
                                    self.root.after(1000, self._auto_seq_step_2_compute)
                            except Exception as e:
                    ...
                    if name == "center_on.jpg" and self._centering_state == 1:
                        try:
                            ...
                            self._centering_state = 2
                            self.ctrl.send({"cmd":"led", "value":0})
                            self._save_undistorted(name, data) # [NEW] Save undistorted
    ...
    # [NEW] Helper to save undistorted image
    def _save_undistorted(self, name, data):
        if self._ud_K is not None:
            try:
                nparr = np.frombuffer(data, np.uint8)
                bgr = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    ...
    def start_auto_sequence(self):
        if self.auto_seq_state != 0:
            ui_q.put(("toast", "⚠️ Auto Sequence already running!"))
            return
        
        ui_q.put(("toast", "🚀 Starting Auto Sequence..."))
        self.auto_seq_state = 1 # SCAN
        self.start_scan()

    def _auto_seq_step_2_compute(self):
        if self.auto_seq_state != 1: return
        ui_q.put(("toast", "🔄 Auto: Computing Target..."))
        # Use the CSV path generated during scan
        if hasattr(self, "_scan_csv_path") and self._scan_csv_path:
            self.point_csv_path.set(str(self._scan_csv_path))
            self.pointing_compute()
            
            # Check if compute was successful (target set)
            # If successful, move to next step
            self.auto_seq_state = 2
            self.root.after(1000, self._auto_seq_step_3_move)
        else:
            ui_q.put(("toast", "❌ Auto: No CSV found!"))
            self.auto_seq_state = 0

    def _auto_seq_step_3_move(self):
        if self.auto_seq_state != 2: return
        ui_q.put(("toast", "🔄 Auto: Moving to Target..."))
        self.pointing_move()
        self.auto_seq_state = 3
        # Wait for move to settle (3s)
        self.root.after(3000, self._auto_seq_step_4_centering)

    def _auto_seq_step_4_centering(self):
        if self.auto_seq_state != 3: return
        ui_q.put(("toast", "🔄 Auto: Starting Centering..."))
        self.centering_enable.set(True)
        self.on_centering_toggle() # Starts the loop
        self.auto_seq_state = 4

    def _auto_seq_step_5_pointing(self):
        if self.auto_seq_state != 4: return
        ui_q.put(("toast", "🔄 Auto: Starting Pointing..."))
        self.pointing_enable.set(True)
        self.on_pointing_toggle() # Starts the loop
        self.auto_seq_state = 5
